...
Table.prototype.toString = function () {
  ...
  function extractColumnWidths(arr, offset) {
    ...
    for (var i = 0; i < arr.length; i++) {
      // make sure holes and out of bounds are calculated as empty strings
      var cell = i in arr ? arr[i] : ""
  ...
  function get_width(obj) {
    return typeof obj === "object" && obj !== null && obj.width != undefined
      ? obj.width
  ...
  function generateRow(items, style) {
    ...
    // using colWidths.length instead of items.length to render empty
    // cells at the end of rows that are shorter than the longest row
    // as colWidths.length is equal to the number of total columns
    for (var i = 0; i < colWidths.length; i++) {
      // make sure holes and out of bounds are displayed as empty strings
      var item = i in items ? items[i] : ""
      var contents = ("" + item)
        .replace(/\r\n|\r/g, "\n")
        .split("\n")
